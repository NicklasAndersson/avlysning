"""Parser för Norra Åsum-formatet: prosa-datumspann på svenska.

Kännetecken:
    Norra åsum övningsfält
    Tillträdes förbud Norra Åsum Övningsfält april-maj 2026:
...
# Detektering
_NORRA_ASUM_RE = re.compile(r"Norra\s+[åÅa]sum", re.IGNORECASE)
...
_SPAN_RE = re.compile(
    ...
    r"(\d{1,2})\s+"                          # startdag
    r"(\w+)\s+"                              # startmånad
    r"kl\s*:?\s*(\d{1,2}):(\d{2})"          # starttid
    r"\s+till\s+"
    r"(?:Måndag|Tisdag|Onsdag|Torsdag|Fredag|Lördag|Söndag)\s+"
    r"(\d{1,2})\s+"                          # slutdag
    r"(\w+)\s+"                              # slutmånad
    r"kl\s*:?\s*(\d{1,2}):(\d{2})",         # sluttid
    re.IGNORECASE,
)


class NorraAsumParser(PDFParser):
    """Parser för Norra Åsum: prosa-datumspann → dagliga restriktioner."""

    @staticmethod
    def can_parse(text: str) -> bool:
        return bool(_NORRA_ASUM_RE.search(text)) and bool(_KL_TILL_RE.search(text))
